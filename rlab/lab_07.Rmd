...
```{r}
norms <- rnorm(100)
betas <- rbeta(100, 0.1, 0.1)
chars <- sample(c("A", "G", "C", "T"), 100, replace=TRUE)
x <- runif(100)
y <- runif(100, 0, x)
df <- data.frame(x, y)
head(norms)
...
For this, can just use some distirbution without a finite 2nd moment.
...
The ECDFs get closer to the true CDFs as the sample size increases.
...
Of these numbers, the difference in reduction between result1's drug and nodrug means is the greatest. This makes sense because they're generated with different distributions.
...
```{r}
results3 <- simulate.data(n=1000, mu.drug=1.1)
plot.data(results3)
```

With more observations and a larger rate, the histogram bars decrease monotonically. Furthermore, the estimated densities are much smoother. 
...
plot(5:100, results,
     xlab="Sample Size", ylab="Successes", main="Successes vs. Sample Size")
```

It looks like the number of successes gets over 190 at about n=45.
...
plot(mus, results,
     xlab="mu.drug", ylab="Successes", main="Successes vs. Drug Strength")
```

It looks like we observe successes over 190 starting at around mu.drug=2.5.
...
new.sim <- function(){
  ...
  for(i in 1:12){
    results[i] <- rep.sim(n=5, mu.drug=1.5)
  }
...
There were more successes this time around. This makes sense because each trial has more chances to get above our cutoff.
...
test.as.you.go <- function(n.per.day, n.days, a.mean, a.sd, b.mean, b.sd){
  ...
  for(i in 1:n.days){
    results[i] <- test.at.end(n=n.per.day, a.mean=a.mean, a.sd=a.sd, b.mean=b.mean, b.sd=b.sd)
...
Yes, this procedure found a significant difference before it hit 20 trials.
...
The results from the test-at-end t-test are fairly close to a false positive rate of 5%. However, there are significantly more false positives in the test-as-you-go t-test, which makes sense because the samples are smaller and allow for larger standard errors.